...
const errorMessages = {
  ...
  isLength: validationItem => {
    switch (validationItem.params.length) {
      case 1: {
        return(
          `Please enter a value with at least ${validationItem.params[0]} character${(validationItem.params[0] != 1 ? 's' : '')}`
        );
      ...
      case 2: {
        return (
          `Please enter a value between ${validationItem.params[0]} and ${validationItem.params[1]} characters long`
        );
...
errorMessages.addErrorMessage = (type, message) => {
  if (!isString(type)) {
    throw new Error(
      'Winterfell: First parameter of addErrorMessage must be of type string'
    );
  }

  if (!isFunction(message) && !isString(message)) {
    throw new Error(
...
errorMessages.addErrorMessages = messages => {
  if (!isObject(messages)) {
    throw new Error(
      'Winterfell: First parameter of addErrorMessages must be of type object'
    );
  }
...
errorMessages.getErrorMessage = validationItem => {
  const errorMessage = (
    !isUndefined(validationItem.message)
    ? validationItem.message
    : !isUndefined(errorMessages[validationItem.type])
    ? errorMessages[validationItem.type]
    : errorMessages.default
  );

  return isFunction(errorMessage) ? errorMessage(validationItem) : errorMessage;
};
 ...
 */
const setErrorMessage = (type, message) => { errorMessages[type] = message; };
